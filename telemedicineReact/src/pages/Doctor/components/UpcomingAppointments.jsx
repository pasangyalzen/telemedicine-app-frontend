...
import { Calendar, Clock, User, RefreshCw, X, AlertCircle } from "lucide-react";
...
const AppointmentCard = ({ appointment, onAction }) => {
  ...
  const isToday = () => {
    ...
    const apptDate = new Date(appointment.scheduledTime);
  ...
  return (
    <div className="flex justify-between items-center p-5 bg-white rounded-xl shadow-sm mb-4 hover:shadow-md transition-all duration-300 border border-gray-100">
      ...
      <div className={`w-1 self-stretch rounded-full mr-4 ${isToday() ? "bg-blue-500" : "bg-green-400"}`}></div>
