import Sidebar from "../../components/Sidebar"; // Adjust the path based on your project structure
import { Users, UserCheck, UserPlus, UserCog, Bell, ChevronDown, Settings, LogOut } from "lucide-react"; // Icons for styling
import { useState } from "react"; // For managing dropdown state

const AdminDashboard = () => {
  // Placeholder data (replace with API calls)
  const totalUsers = 1000;
  const totalDoctors = 200;
  const totalPatients = 700;
  const totalPharmacists = 100;

  const [dropdownOpen, setDropdownOpen] = useState(false); // Manage dropdown state

  return (
    <div className="flex h-screen w-screen overflow-hidden">
      {/* Sidebar Component */}
      <Sidebar />

      {/* Main Dashboard Content */}
      <div className="flex-1 min-h-screen flex flex-col">
        {/* Top Navbar */}
        <div className="bg-primary-dark text-[#65cccc] p-4 flex justify-between items-center border-b border-gray-700">
          {/* Branding (TELECHAUKI) */}
          <h1 className="text-2xl font-bold">ADMIN DASHBOARD</h1>

          {/* Right-side Navbar Content */}
          <div className="flex items-center space-x-4">
            {/* Notification Icon */}
            <div className="relative">
              <Bell className="w-6 h-6 text-[#65cccc] hover:text-white cursor-pointer" />
              <span className="absolute top-0 right-0 bg-red-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center">3</span>
            </div>

            {/* Admin Dropdown */}
            <div className="relative">
              <button onClick={() => setDropdownOpen(!dropdownOpen)} className="flex items-center space-x-2 text-[#65cccc] hover:text-white">
                <span>Admin</span>
                <ChevronDown className="w-4 h-4" />
              </button>

              {/* Dropdown Menu */}
              {dropdownOpen && (
                <div className="absolute right-0 mt-2 w-40 bg-white text-black rounded-lg shadow-lg">
                  <Link to="/admin/settings" className="block px-4 py-2 hover:bg-gray-200">
                    <Settings className="w-5 h-5 inline mr-2" />
                    Settings
                  </Link>
                  <Link to="/admin/logout" className="block px-4 py-2 hover:bg-gray-200">
                    <LogOut className="w-5 h-5 inline mr-2" />
                    Logout
                  </Link>
                </div>
              )}
            </div>
          </div>
        </div>

        {/* Stats Boxes (Flex Grid) */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 p-6 flex-grow">
          {/* Total Users */}
          <div className="p-6 bg-white rounded-lg shadow-md flex items-center w-full">
            <Users className="w-10 h-10 text-blue-500 mr-4" />
            <div>
              <h3 className="text-3xl font-semibold text-[#49cccc]">Total Users</h3>
              <p className="text-2xl text-[#49cccc] font-bold">{totalUsers}</p>
            </div>
          </div>

          {/* Total Doctors */}
          <div className="bg-white p-6 rounded-lg shadow-md flex items-center w-full">
            <UserCheck className="w-10 h-10 text-green-500 mr-4" />
            <div>
              <h3 className="text-3xl font-semibold text-[#49cccc]">Total Doctors</h3>
              <p className="text-2xl text-[#49cccc] font-bold">{totalDoctors}</p>
            </div>
          </div>

          {/* Total Patients */}
          <div className="bg-white p-6 rounded-lg shadow-md flex items-center w-full">
            <UserPlus className="w-10 h-10 text-purple-500 mr-4" />
            <div>
              <h3 className="text-3xl font-semibold text-[#49cccc]">Total Patients</h3>
              <p className="text-2xl text-[#49cccc] font-bold">{totalPatients}</p>
            </div>
          </div>

          {/* Total Pharmacists */}
          <div className="bg-white p-6 rounded-lg shadow-md flex items-center w-full">
            <UserCog className="w-10 h-10 text-[#ff6b6b] mr-4" />
            <div>
              <h3 className="text-3xl font-semibold text-[#49cccc]">Total Pharmacists</h3>
              <p className="text-2xl text-[#49cccc] font-bold">{totalPharmacists}</p>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default AdminDashboard;
